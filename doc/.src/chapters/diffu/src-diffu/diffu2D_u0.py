...
def solver_dense(
    ...
    if f is None or f == 0:
        f = lambda x, y, t: np.zeros((x.size, y.size)) if isinstance(x, np.ndarray) else 0
    ...
    for n in It[0:-1]:
        ...
        for i in Ix:
            p = m(i, j)
            b[p] = U_0y(t[n + 1])  # boundary
        for j in Iy[1:-1]:
            i = 0
            p = p = m(i, j)
            b[p] = U_0x(t[n + 1])  # boundary
            for i in Ix[1:-1]:
                p = m(i, j)  # interior
                b[p] = (
                    u_n[i, j]
            ...
            i = Nx
            p = m(i, j)
            b[p] = U_Lx(t[n + 1])  # boundary
        j = Ny
        for i in Ix:
            p = m(i, j)
            b[p] = U_Ly(t[n + 1])  # boundary
        # print b
...
def solver_sparse(
    ...
    m = lambda i, j: j * (Nx + 1) + i
    j = 0
    main[m(0, j) : m(Nx + 1, j)] = 1  # j=0 boundary line
    for j in Iy[1:-1]:  # Interior mesh lines j=1,...,Ny-1
        i = 0
        main[m(i, j)] = 1  # Boundary
        i = Nx
        main[m(i, j)] = 1  # Boundary
    ...
    j = Ny
    main[m(0, j) : m(Nx + 1, j)] = 1  # Boundary line

    A = scipy.sparse.diags(
        diagonals=[main, lower, upper, lower2, upper2],
        offsets=[0, -lower_offset, lower_offset, -lower2_offset, lower2_offset],
        shape=(N, N),
        format="csc",
    )
    ...
    for n in It[0:-1]:
        ...
        j = 0
        b[m(0, j) : m(Nx + 1, j)] = U_0y(t[n + 1])  # Boundary
        for j in Iy[1:-1]:
            i = 0
            p = m(i, j)
            b[p] = U_0x(t[n + 1])  # Boundary
        ...
        j = Ny
        b[m(0, j) : m(Nx + 1, j)] = U_Ly(t[n + 1])  # Boundary
        ...
        if method == "direct":
            ...
            c, info = scipy.sparse.linalg.cg(
                A, b, x0=x0, tol=CG_tol, maxiter=N, M=M, callback=CG_callback
            )
            """
            if info > 0:
                print('CG: tolerance %g not achieved within %d iterations'                       % (CG_tol, info))
            elif info < 0:
                print('CG breakdown')
            else:
                print('CG converged in %d iterations (tol=%g)'                       % (CG_iter[-1], CG_tol))
            """
        # Fill u with vector c
...
def solver_classic_iterative(
    ...
    if version == "vectorized" and iteration == "SOR":
        if (Nx % 2) != 0 or (Ny % 2) != 0:
            raise ValueError("Vectorized SOR requires even Nx and Ny (%dx%d)" % (Nx, Ny))
    if version == "SOR" and omega == "optimal":
    ...
    for n in It[0:-1]:
        ...
        while not converged:
            if version == "scalar":
                ...
                for j in Iy[1:-1]:
                    i = 0
                    u[i, j] = U_0x(t[n + 1])  # Boundary
                    i = Nx
                    u[i, j] = U_Lx(t[n + 1])  # Boundary
                    for i in Ix[1:-1]:
                        u_new = (
                            1.0
                            / (1.0 + 2 * theta * (Fx + Fy))
                ...
                j = 0
                u[:, j] = U_0y(t[n + 1])  # boundary
                i = 0
                u[i, :] = U_0x(t[n + 1])  # boundary
                i = Nx
                u[i, :] = U_Lx(t[n + 1])  # boundary
                j = Ny
                u[:, j] = U_Ly(t[n + 1])  # boundary
                # Internal points
                f_a_np1 = f(xv, yv, t[n + 1])
                f_a_n = f(xv, yv, t[n])
                ...
                if iteration == "Jacobi":
                    ...
                    jm1 = slice(0, -2, 2)
                    jp1 = slice(2, None, 2)
                    u_new[ic, jc] = update(u_new, u_n, ic, im1, ip1, jc, jm1, jp1)
        ...
        print(
            "t=%.2f: %s %s (omega=%g) finished in %d iterations"
...
def quadratic(theta, Nx, Ny):
    ...
    t, cpu = solver_dense(
        I, a, f, Lx, Ly, Nx, Ny, dt, T, theta, user_action=assert_no_error
    )
    ...
    for iteration in "Jacobi", "SOR":
        for version in "scalar", "vectorized":
            for theta in 1, 0.5:
                print(
                    f"\ntesting {iteration}, {version} version, theta={theta:g}, tol={tol:g}"
                )
    ...
    print(f"\ntesting CG+ILU, theta={theta:g}, tol={tol:g}")
    solver_sparse(
        I,
        a,
        f,
    ...
    print(f"\ntesting CG, theta={theta:g}, tol={tol:g}")
    solver_sparse(
        I,
        a,
        f,
...
def demo_classic_iterative(
    tol=1e-4, iteration="Jacobi", version="vectorized", theta=0.5, Nx=10, Ny=10
):
    Lx = 2.0
    Ly = 1.0
    a = 1.5
    ...
    u_exact = (
        lambda x, y, t: np.exp(-a * np.pi**2 * (Lx ** (-2) + Ly ** (-2)) * t)
        * np.sin(np.pi * x / Lx)
        * np.sin(np.pi * y / Ly)
    )
    I = lambda x, y: u_exact(x, y, 0)
    f = lambda x, y, t: 0 if isinstance(x, (float, int)) else np.zeros((Nx + 1, Ny + 1))
    dt = 0.2
    dt = 0.05
    T = 0.5
    ...
    def examine(u, x, xv, y, yv, t, n):
        ...
        if theta == 1:
            ...
            A_d = (
                (1 - 2 * Fx * np.sin(px) ** 2 - 2 * Fy * np.sin(py) ** 2)
                / (1 + 2 * Fx * np.sin(px) ** 2 + 2 * Fy * np.sin(py) ** 2)
            ) ** n
        ...
        print(
            f"Max u: {u.max():.2E}",
            f"error in u: {u_diff:.2E}",
            f"ampl.: {A_diff:.2E}",
            f"iter: {abs(u_diff - A_diff):.2E}",
        )
...
def convergence_rates(theta, num_experiments=6):
    ...
    def f(x, y, t):
        return (
            (-p + a * (kx**2 + ky**2)) * np.exp(-p * t) * np.sin(kx * x) * np.sin(ky * y)
        )
    ...
    def assert_conv_rates():
        r = [
            np.log(E_values[i + 1] / E_values[i]) / np.log(h_values[i + 1] / h_values[i])
            for i in range(0, num_experiments - 2, 1)
        ]
        tol = 0.5
        if theta == 0:  # i.e., FE
            diff = abs(r_FE_BE_expected - r[-1])
            msg = f"Forward Euler. r = 1 expected, got={r[-1]:g}"
        elif theta == 1:  # i.e., BE
            diff = abs(r_FE_BE_expected - r[-1])
            msg = f"Backward Euler. r = 1 expected, got={r[-1]:g}"
        else:  # theta == 0.5, i.e, CN
            diff = abs(r_CN_expected - r[-1])
            msg = f"Crank-Nicolson. r = 2 expected, got={r[-1]:g}"
            # print msg
    ...
    for method in "direct", "CG":
        print(f"\ntesting convergence rate, theta={theta:g}, method={method}")
        for i in range(num_experiments):
            ...
            N = 2 ** (i + 1)
            Nx = N
            Ny = N  # We want dx=dy, so Nx=Ny if Lx=Ly
            dx = float(Lx) / N
    ...
    def assert_correct_convergence_rate(u, x, xv, y, yv, t, n):
        ...
        if t[n] == T:  # hpl: dangerous comparison...
            ...
            if counter["i"] == num_experiments:  # i.e., all num. exp. finished
                ...
                r = [
                    np.log(E_values[i + 1] / E_values[i])
...
def efficiency():
    ...
    t, cpu = solver_sparse(
        I, a, f, Lx, Ly, Nx, Ny, dt, T, theta, user_action=None, method="direct"
    )
