...
class SerialLayers:
    ...
    def __init__(self, a, b, U_0, U_L, eps=0):
        ...
        a_data = [[bi, ai] for bi, ai in zip(self.b, self.a, strict=False)]
        domain = [b[0], b[-1]]
        self.a_func = PiecewiseConstant(domain, a_data, eps)

        # inv_a = 1/a is needed in formulas
        inv_a_data = [[bi, 1.0 / ai] for bi, ai in zip(self.b, self.a, strict=False)]
        self.inv_a_func = PiecewiseConstant(domain, inv_a_data, eps)
        self.integral_of_inv_a_func = IntegratedPiecewiseConstant(domain, inv_a_data, eps)
    ...
    def u_exact(self, x):
        solution = (
            self.U_0
            + (self.U_L - self.U_0) * self.integral_of_inv_a_func(x) / self.inv_a_0L
        )
        return solution
    ...
    def plot(self):
        ...
        if self.eps > 0:
            plt.title(f"Smoothing eps: {self.eps}")
        plt.savefig("tmp.pdf")
        plt.savefig("tmp.png")
        plt.show()
