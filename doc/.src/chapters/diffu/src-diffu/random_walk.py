...
def demo_random_walk1D_timing():
    ...
    t0 = time.clock()
    np.random.seed(10)
    random_walk1D(x0, N, p, random=np.random)
    t1 = time.clock()
    cpu_scalar = t1 - t0
    print(f"CPU scalar: {cpu_scalar:.1f}")
    np.random.seed(10)
    random_walk1D_vec(x0, N, p)
    t2 = time.clock()
    cpu_vec = t2 - t1
    print(f"CPU vectorized: {cpu_vec:.1f}")
...
def test_random_walks1D():
    ...
    for s, v, r in zip(
        serial_computed, vectorized1_computed, return_values, strict=False
    ):
        msg = f"{r}: {s} (serial) vs {v} (vectorized)"
        assert (s == v).all(), msg
    for s, v, r in zip(
        serial_computed, vectorized2_computed, return_values, strict=False
    ):
        msg = f"{r}: {s} (serial) vs {v} (vectorized)"
        assert (s == v).all(), msg
...
def demo_random_walks1D_timing():
    ...
    t2 = time.clock()
    cpu_vec1 = t2 - t1
    print(f"CPU vectorized1: {cpu_vec1:.1f}")
    ...
    t3 = time.clock()
    cpu_vec2 = t3 - t2
    print(f"CPU vectorized2: {cpu_vec2:.1f}")
...
def demo_random_walkdD_timing():
    ...
    t0 = time.clock()
    np.random.seed(10)
    random_walkdD(x0, N, p, random=np.random)
    t1 = time.clock()
    cpu_scalar = t1 - t0
    print(f"CPU scalar: {cpu_scalar:.1f}")
    np.random.seed(10)
    random_walkdD_vec(x0, N, p)
    t2 = time.clock()
    cpu_vec = t2 - t1
    print(f"CPU vectorized: {cpu_vec:.1f}")
    print("CPU scalar/vectorized: %.1f" % (cpu_scalar / cpu_vec))
...
def test_random_walksdD():
    ...
    return_values = ["pos", "pos2", "pos_hist", "pos_hist_times"]
    for s, v, r in zip(serial_computed, vectorized_computed, return_values, strict=False):
        msg = f"{r}: {s.shape}\n{s} (serial)\nvs\n{v.shape}\n{v} (vectorized)"
        assert (s == v).all(), msg
