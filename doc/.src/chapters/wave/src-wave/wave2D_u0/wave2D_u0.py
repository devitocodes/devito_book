...
def solver(I, V, f, c, Lx, Ly, Nx, Ny, dt, T, user_action=None, version="scalar"):
    ...
    if dt <= 0:  # max time step?
        ...
        print(f"error: dt={dt:g} exceeds the stability limit {stability_limit:g}")
    ...
    if f is None or f == 0:
        f = (
            (lambda x, y, t: 0)
            if version == "scalar"
            else lambda x, y, t: np.zeros((x.shape[0], y.shape[1]))
        )
        # or simpler: x*y*0
    if V is None or V == 0:
        V = (
            (lambda x, y: 0)
            if version == "scalar"
            else lambda x, y: np.zeros((x.shape[0], y.shape[1]))
        )
    ...
    if version == "scalar":
        u = advance_scalar(u, u_n, u_nm1, f, x, y, t, n, Cx2, Cy2, dt2, V, step1=True)

    else:
        f_a[:, :] = f(xv, yv, t[n])  # precompute, size as u
        V_a = V(xv, yv)
        u = advance_vectorized(u, u_n, u_nm1, f_a, Cx2, Cy2, dt2, V=V_a, step1=True)
    ...
    for n in It[1:-1]:
        ...
        if user_action is not None and user_action(u, x, xv, y, yv, t, n + 1):
            break
...
def advance_vectorized(u, u_n, u_nm1, f_a, Cx2, Cy2, dt2, V=None, step1=False):
    if step1:
        dt = np.sqrt(dt2)  # save
        Cx2 = 0.5 * Cx2
        Cy2 = 0.5 * Cy2
...
def quadratic(Nx, Ny, version):
    ...
    new_dt, cpu = solver(
        I, V, f, c, Lx, Ly, Nx, Ny, dt, T, user_action=assert_no_error, version=version
    )
    return new_dt, cpu
...
def run_efficiency(nrefinements=4):
    ...
    c = 1.5
    T = 100
    versions = ["scalar", "vectorized", "cython", "f77", "c_f2py", "c_cy"]
    print(" " * 15, "".join(["%-13s" % v for v in versions]))
    for Nx in 15, 30, 60, 120:
        cpu = {}
        for version in versions:
            dt, cpu_ = solver(
                I, None, None, c, Lx, Ly, Nx, Nx, -1, T, user_action=None, version=version
            )
            cpu[version] = cpu_
        ...
        if cpu_min < 1e-6:
            ...
            print("".join([f"{cpu[version]:13.1f}" for version in versions]))
...
def gaussian(plot_method=2, version="vectorized", save_plot=True):
    ...
    def plot_u(u, x, xv, y, yv, t, n):
        ...
        if plot_method == 1:
            ...
            st.mesh(x, y, u, title=f"t={t[n]:g}", zlim=[-1, 1], caxis=[-1, 1])
        elif plot_method == 2:
            # Works well with Gnuplot backend, not with Matplotlib
            st.surfc(
                xv,
            ...
            mlab.outline(s, color=(0.7, 0.7, 0.7), extent=extent1)
    ...
    T = 20
    dt, cpu = solver(
        I, None, None, c, Lx, Ly, Nx, Ny, -1, T, user_action=plot_u, version=version
    )
