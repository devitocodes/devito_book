...
class PlotAndStoreSolution:
    ...
    def __call__(self, u, x, t, n):
        ...
        if self.backend is None:
            ...
            self.plt.plot(
                ...
                xlabel="x",
                ylabel="u",
                axis=[x[0], x[-1], self.yaxis[0], self.yaxis[1]],
                title=title,
    ...
    def make_movie_file(self):
        ...
        if self.backend is not None:
            ...
            movie("frame_*.png", encoder="html", output_file="index.html", fps=fps)
    ...
    def close_file(self, hashed_input):
        ...
        if self.filename is not None:
            ...
            np.savez("." + self.filename + "_t.dat", t=np.array(self.t, dtype=float))
...
def moving_end(C=1, Nx=50, reflecting_right_boundary=True, version="vectorized"):
    # Scaled problem: L=1, c=1, max I=1
    L = 1.0
    c = 1
    dt = (L / Nx) / c  # choose the stability limit with given Nx
    ...
    def U_0(t):
        return 1.0 * sin(6 * np.pi * t) if t < 1.0 / 3 else 0
    ...
    action = PlotAndStoreSolution(
        ...
        2.3,
        skip_frame=4,
        title="u(0,t)=0.25*sin(6*pi*t) if t < 1/3 else 0, " + bc_right,
        filename="tmpdata",
    )
    cpu, hashed_input = solver(
        I,
...
def animate_multiple_solutions(*archives):
    [load(archive) for archive in archives]
...
def pulse(
    ...
    medium=None,  # interval for right medium
    ...
    if pulse_tp in ("gaussian", "Gaussian"):
        ...
        def I(x):
            ...
            a = w * sigma
            return (
                0.5 * (1 + np.cos(np.pi * (x - xc) / a)) if xc - a <= x <= xc + a else 0
            )
        ...
        def I(x):
            ...
            a = w * sigma
            return (
                np.cos(np.pi * (x - xc) / a) if xc - 0.5 * a <= x <= xc + 0.5 * a else 0
            )
    else:
        raise ValueError(f'Wrong pulse_tp="{pulse_tp}"')

    def c(x):
        return c_0 / slowness_factor if medium[0] <= x <= medium[1] else c_0

    umin = -0.5
    umax = 1.5 * I(xc)
    casename = f"{pulse_tp}_Nx{Nx}_sf{slowness_factor}"
