...
def solver(I, U0, v, L, dt, C, T, user_action=None, scheme="FE", periodic_bc=True):
    ...
    for n in range(0, Nt):
        if scheme == "FE":
            ...
            raise ValueError(f'scheme="{scheme}" not implemented')
...
def run(scheme="UP", case="gaussian", C=1, dt=0.01):
    ...
    def plot(u, x, t, n):
        ...
        if n == 0:
            ...
            lines[0].set_ydata(u)
            plt.axis([x[0], x[-1], -0.5, 1.5])
            plt.title(f"C={C:g}, dt={t[1] - t[0]:g}, dx={x[1] - x[0]:g}")
            plt.legend([f"t={t[n]:.3f}"])
            plt.xlabel("x")
            plt.ylabel("u")
            plt.draw()
            plt.savefig("tmp_%04d.png" % n)
        ...
        if n > 0:
            ...
            plt.plot(x, y, "k--")
            if abs(t[n] - 0.6) < eps:
                filename = (f"tmp_{scheme}_dt{t[1] - t[0]}_C{C}").replace(".", "")
    ...
    for codec in codecs:
        cmd = f"ffmpeg -i tmp_%04d.png -r 25 -vcodec {codecs[codec]} movie.{codec}"
        os.system(cmd)
...
def solver_theta(I, v, L, dt, C, T, theta=0.5, user_action=None, FE=False):
    ...
    time.clock()  # for measuring the CPU time
    Nt = int(round(T / float(dt)))
    t = np.linspace(0, Nt * dt, Nt + 1)  # Mesh points in time
    dx = v * dt / C
    Nx = int(round(L / dx))
