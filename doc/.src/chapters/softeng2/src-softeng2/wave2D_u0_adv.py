...
def solver(I, V, f, c, Lx, Ly, Nx, Ny, dt, T, user_action=None, version="scalar"):
    ...
    if f is None or f == 0:
        f = (
            (lambda x, y, t: 0)
            if version == "scalar"
            else lambda x, y, t: zeros((x.shape[0], y.shape[1]))
        )
        # or simpler: x*y*0
    if V is None or V == 0:
        V = (
            (lambda x, y: 0)
            if version == "scalar"
            else lambda x, y: zeros((x.shape[0], y.shape[1]))
        )
...
def gaussian(plot_method=2, version="vectorized", save_plot=True):
    ...
    def plot_u(u, x, xv, y, yv, t, n):
        if t[n] == 0:
            time.sleep(2)
        if plot_method == 1:
            mesh(x, y, u, title=f"t={t[n]:g}", zlim=[-1, 1], caxis=[-1, 1])
        elif plot_method == 2:
            surfc(
                xv,
                yv,
