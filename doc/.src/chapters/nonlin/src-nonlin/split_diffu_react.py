...
def Strang_splitting_1stOrder(I, a, b, f, L, dt, dt_Rfactor, F, t, T, user_action=None):
    """Strange splitting while still using FE for the reaction
...
def Strang_splitting_2ndOrder(I, a, b, f, L, dt, dt_Rfactor, F, t, T, user_action=None):
    ...
    Nt = int(round(T / float(dt)))
    np.linspace(0, Nt * dt, (Nt + 1) + Nt)  # Mesh points in diff
    dx = np.sqrt(a * dt / F)
    Nx = int(round(L / dx))
    x = np.linspace(0, L, Nx + 1)
    u = np.zeros(Nx + 1)
...
def convergence_rates(scheme="diffusion"):
    ...
    for Nx in Nx_values:
        ...
        if scheme == "Strang_splitting_2ndOrder":
            print("Strange splitting with 2nd order schemes...")
            ...
            if scheme == "diffusion":
                ...
                print("Strange splitting with 1st order schemes...")
