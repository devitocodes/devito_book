...
# Parameters for the initial surface elevation (arguments to g)
I_m = 0
I_s = 1.5
I_a = 1.6
I_0 = 0
...
left_boundary = (
    Line((I_m, B_0), (I_m, ymax)).set_linestyle("dotted").set_linecolor("black")
)

B_m_marker = Distance_wText(
    (I_m, B_0 - 0.5 * H_0), (B_m, B_0 - 0.5 * H_0), "$B_m$", text_spacing=-1.0 / 30
)
H_0_marker = Distance_wText((B_m + 4 * B_s, B_0), (B_m + 4 * B_s, B_0 + H_0), "$H_0$")
B_a_marker1 = Distance_wText((B_m - 3 * B_s, B_0), (B_m - 3 * B_s, B_0 + B_a), "$B_a$")
B_a_marker2 = Line((B_m - 3 * B_s, B_0 + B_a), (B_m, B_0 + B_a)).set_linestyle("dotted")
L_marker = Distance_wText(
    (B_m, B_0 - 0.5 * H_0), (xmax, B_0 - 0.5 * H_0), "$4mB_s$", text_spacing=-1.0 / 30
)
x0 = Text("$x=0$", (0, B_0 - H_0 / 4.0))
...
markers = Composition(dict(H_0=H_0_marker, x0=x0, left=left_boundary))
fig1 = Composition(dict(bottom=bottom_flat, surface=surface, markers=markers, Ixy=Ixy))
...
curves = Composition(dict(bottom=bottom_flat, surface_L=surface_L, surface_R=surface_R))
al = I_0 + 0.5 * I_R_a  # arrow level
arrow_R = (
    Arrow1((x_R + 0.5 * I_s, al), (x_R + 1.5 * I_s, al))
    .set_linecolor("black")
    .set_linewidth(1)
)
arrow_L = (
    Arrow1((x_L - 0.5 * I_s, al), (x_L - 1.5 * I_s, al))
    .set_linecolor("black")
    .set_linewidth(1)
)
fig2 = Composition(
    dict(
        curves=curves,
        markers=markers,
        arrows=Composition(dict(arrow_L=arrow_L, arrow_R=arrow_R)),
    )
)
drawing_tool.erase()
fig2.draw()
...
markers_I = Ixy
markers_B = Composition(
    dict(
        B_m=B_m_marker,
        B_a=Composition(dict(B_a_dist=B_a_marker1, B_a_dotted=B_a_marker2)),
        L=L_marker,
        Bxy=Bxy,
    )
)
fig3 = Composition(
    dict(curves=curves, markers=markers, markers_I=markers_I, markers_B=markers_B)
)
