...
$$
[D_tD_t u = D_x\overline{q}^xD_x u + f]^n_i \tp
$$
...
is worked out are described in ref[Sections @eq-wave-pde2-Neumann
and @eq-wave-pde2-var-c][ in [@Langtangen_deqbook_wave]][in
...
(@eq-wave-pde2-software-ueq2)-(@eq-wave-pde2-software-bcL2)
...
@eq-wave-pde2-Neumann, and @eq-wave-pde2-var-c][ in
...
{#sec-softeng2-wave1D-filestorage}
...
{#sec-softeng2-wave1D-filestorage-savez}
...
{#sec-softeng2-wave1D-filestorage-joblib}
...
class Storage:
    ...
    def retrieve(self, name, data=None):
        ...
        return data
```
The `retrieve` and `save` functions, which do the work, seem quite
...
see Section 2.1.4 in [@Langtangen_scaling] for a brief explanation]).

## Using a hash to create a file or directory name
{#sec-softeng2-wave1D-filestorage-hash}

Array storage techniques like those outlined in Sections
@eq-softeng2-wave1D-filestorage-joblib and
@eq-softeng2-wave1D-filestorage-savez demand the user to assign a
...
:::{.callout-warning title="Remark: turning function objects into their source code is unreliable!"}
...
one archive. This is the subject of Exercise @sec-softeng2-exer-savez.


## Making hash strings from input data
{#sec-softeng2-wave1D-filestorage-hashlib}
...
{#sec-softeng2-wave1D-filestorage-norerun}
...
$h_{i-1}$. ref[Section @eq-wave-pde2-fd-standing-waves][ in [@Langtangen_deqbook_wave]][The
...
analysis in ref[Section @eq-wave-pde1-analysis][in
...
:::{.callout-note title="We rely on attribute access - not get/set functions!"}
...
code with loops into Cython.
 ...
 1. Variables are declared with types: for example,
   ...
   declared by `int`.
 1. Arrays need a comprehensive type declaration involving
   * the type `np.ndarray`,
 ...
 1. Functions declared with `cpdef` are translated to C but are also
   accessible from Python.
 1. In addition to the standard `numpy` import we also need a special
   Cython import of `numpy`: `cimport numpy as np`, to appear *after*
   the standard import.
 1. By default, array indices are checked to be within their legal
 ...
 1. Also by default, array indices can be negative (counting from the
   ...
   the function header.
 1. The use of index sets `Ix` and `It` in the scalar code cannot be
...
:::{.callout-note title="Array declaration syntax in Cython"}
...
translated to C. Figure @fig-wave-2D3D-impl-fig-cython-ma1 shows
...
![Visual illustration of Cython's ability to translate Python to C.](fig/wave2D_u0_loop_cy1){#fig-wave-2D3D-impl-fig-cython-ma1 width="500px"}
...
:::{.callout-note title="Simplified build of a Cython module"}
...
(@sec-wave-2D3D-impl1-2Du0-ueq-discrete) and setting the solution to zero
at the boundaries:
...
:::{.callout-note title="Tip: Return all computed objects to the calling code"}
...
:::{.callout-warning title="Examine the doc strings!"}
...
:::{.callout-warning title="Be careful with macro definitions"}
...
Section @eq-softeng2-wave1D-filestorage provides a recipe for
