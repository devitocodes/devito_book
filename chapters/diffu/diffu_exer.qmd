...
if the initial data exhibit jumps (see Section @eq-diffu-pde1-analysis-CN).
...
{#sec-diffu-exer-energy-estimates}
file=diffu_energy
...
The estimate (@sec-diffu-exer-estimates-p1-result) says that the
``size of $u$'' never exceeds that of the initial condition,
or more precisely, it says that the area under the $u$ curve decreases
with time.

To show (@sec-diffu-exer-estimates-p1-result), multiply the PDE
...
(@sec-diffu-exer-estimates-p1-result).
...
(@sec-diffu-exer-estimates-p1-pdf)-(@sec-diffu-exer-estimates-p1-ic)
and $w_2$ fulfills
(@sec-diffu-exer-estimates-p2-pdf)-(@sec-diffu-exer-estimates-p2-ic),
then $u=w_1 + w_2$ is the solution of
(@sec-diffu-exer-estimates-p3-pdf)-(@sec-diffu-exer-estimates-p3-ic).
...
!bsubex
One application of (@sec-diffu-exer-estimates-p3-result) is to prove uniqueness
of the solution.
Suppose $u_1$ and $u_2$ both fulfill
(@sec-diffu-exer-estimates-p3-pdf)-(@sec-diffu-exer-estimates-p3-ic).
Show that $u=u_1 - u_2$ then fulfills
(@sec-diffu-exer-estimates-p3-pdf)-(@sec-diffu-exer-estimates-p3-ic)
with $f=0$ and $I=0$. Use (@sec-diffu-exer-estimates-p3-result)
to deduce that the energy must be zero for all times and therefore
that $u_1=u_2$, which proves that the solution is unique.
!esubex

!bsubex
Generalize (@sec-diffu-exer-estimates-p3-result) to a 2D/3D
...
We can use (@sec-diffu-exer-estimates-p3-result)
and (@sec-diffu-exer-estimates-p4-result) as a partial verification
of the code. Choose some functions $f$ and $I$ and
check that (@sec-diffu-exer-estimates-p3-result) is obeyed at any
time when zero Dirichlet conditions are used.
Iterate over the same $I$ functions and check that
(@sec-diffu-exer-estimates-p4-result) is fulfilled
...
the energy estimates and the estimate (@sec-diffu-exer-estimates-p4-result)
are for pointing out errors in the implementation.
!esubex

## Exercise: Splitting methods and preconditioning
{#sec-diffu-exer-splitting-prec}

In Section @eq-diffu-2D-direct-vs-iter, we outlined a class of
...
$k-1$. The formula (@sec-diffu-exer-splitting-prec-simplest) is known
as Richardson's iteration.
Show that if we apply the simple iteration method
(@sec-diffu-exer-splitting-prec-simplest) to the *preconditioned*
...
$$
which is (@sec-diffu-exer-splitting-prec-simplest).
...
{#sec-diffu-exer-axisymm}
file=FE_axisym

Based on the discussion in Section @eq-diffu-fd2-radial, derive in detail
...
!bsol
We start with the equation at $r=0$. According to Section @eq-diffu-fd2-radial,
we get
