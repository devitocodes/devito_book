## Applications {#sec-diffu-app}

### Diffusion of a substance {#sec-diffu-app-substance}
...
@fig-diffu-app-substance-fig1.  We realize that only the normal
...
\int\limits_{\partial V} \varrho\q\cdot\normalvec\, dS = 0\tp
$$ {#eq-diffu-app-substance-integral}
To arrive at a PDE,
...
\frac{\partial c}{\partial t} +
\nabla\cdot \q = 0\tp
$$ {#eq-diffu-app-substance-massconv}

![An arbitrary volume of a fluid.](fig/continuum){#fig-diffu-app-substance-fig1 width="200px"}
...
$$
\q = -\dfc\nabla c,
$$ {#eq-diffu-app-substance-Fick}
where $\dfc$ is an empirically determined constant.
The relation (@eq-diffu-app-substance-Fick) is known as Fick's law.
Inserting (@eq-diffu-app-substance-Fick) in
(@eq-diffu-app-substance-massconv) gives a scalar PDE for the
concentration $c$:
$$
\frac{\partial c}{\partial t} = \dfc\nabla^2 c\tp
$$ {#eq-diffu-app-substance-PDE}

### Heat conduction {#sec-diffu-app-heat}
...
Figure @fig-diffu-app-substance-fig1, the corresponding increase in internal
...
$$
\varrho  c_v\frac{\partial T}{\partial t} = \nabla\cdot(k\nabla T) + f\tp
$$ {#eq-diffu-app-heat-PDE}

### Porous media flow {#sec-diffu-app-porous}
...
$$
S\frac{\partial p}{\partial t} + \nabla\cdot(\q - \alpha\frac{\partial\uu}{\partial t}) = 0,
$$
where $p$ is the fluid pressure, $\q$ is the fluid velocity, $\uu$
...
In many circumstances, the last term with $\uu$ can be neglected,
...
Let $\vv$ be the velocity of a fluid. The condition $\nabla\times\vv =0$
is relevant for many flows, especially in geophysics when viscous effects
are negligible. From vector calculus it is known
that $\nabla\times\vv =0$ implies that $v$ can be derived
from a scalar potential field $\phi$: $\vv = \nabla\phi$. If the
fluid is incompressible, $\nabla\cdot\vv = 0$, it follows that
$\nabla\cdot\nabla\phi = 0$, or
$$
\nabla^2\phi = 0\tp
$$
...
$$
It follows that $\nabla\vv = \psi_{yx} - \psi_{xy}=0$, so the stream function
can only be used for incompressible flows.
Since
$$
\nabla\times\vv = \left(\frac{\partial v}{\partial y} - \frac{\partial u}{\partial x}\right)\kk \equiv \omega\kk,
$$
...
### Development of flow between two flat plates {#sec-diffu-app-Couette}
...
distance $x$ from the plates; $\uu = u(x,t)\kk$.  The flow is governed
by the Navier-Stokes equations,

\begin{align*}
\varrho\frac{\partial\uu}{\partial t} + \varrho\uu\cdot\nabla\uu &= -\nabla p
+ \mu\nabla^2\uu + \varrho\f,\\
\nabla\cdot\uu &= 0,
\end{align*}
where $p$ is the pressure field, unknown along with the velocity $\uu$,
...
two flat plates puts restrictions on the velocity, $\uu = u(x,t)\ii$,
...
the plates, which means $\uu=0$ at the plates. Say the location of the
...
in Section @sec-diffu-app-Couette, it is natural to assume that the
velocity is directed along the tube, and that it is axi-symmetric.
These assumptions reduced the velocity field to $\uu = u(r,x,t)\ii$, if the
$x$ axis is directed along the tube. From the equation of continuity,
$\nabla\cdot\uu = 0$, we see that $u$ must be independent of $x$.
Inserting $\uu = u(r,t)\ii$ in the Navier-Stokes equations, expressed
...
in Section @sec-diffu-app-Couette as $t\rightarrow\infty$. The limit
problem can be solved analytically, and the local volume flux becomes
$$
\q(x,y,z) = \int\limits_0^{h}\uu(x,y,z) dz
