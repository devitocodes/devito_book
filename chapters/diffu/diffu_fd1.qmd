...
do in Section @eq-diffu-pde1-FEsec, but the time step restrictions
...
equations from Chapter @eq-ch-wave. Readers not familiar with the
...
time $t$. Other applications are listed in Section @eq-diffu-app.
...
$F$ (see Section @eq-diffu-pde1-analysis for details).
:::
...
Section @eq-diffu-pde1-analysis.

## Implementation
{#sec-diffu-pde1-FE-code}
...
$$
With the formulas from Appendix @sec-sec-form-fdtn we can easily check
...
{#sec-diffu-pde1-FE-verify-convrates}
...
As previously,
in Section @eq-wave-pde2-fd-MMS,
...
@fig-diffu-pde1-FE-fig-F=0.5 at four time levels.
...
Figure @fig-diffu-pde1-FE-fig-F=0.25 (and a
"movie": "{{< var docraw >}}/mov-diffu/diffu1D_u0_FE_plug_F025/movie.ogg").

Increasing $F$ slightly beyond the limit 0.5, to $F=0.51$,
gives growing, non-physical instabilities,
as seen in Figure @fig-diffu-pde1-FE-fig-F=0.51.

![Forward Euler scheme for $F=0.5$.](fig/plug_FE_F05){#fig-diffu-pde1-FE-fig-F=0.5 width="800px"}

![Forward Euler scheme for $F=0.25$.](fig/plug_FE_F025){#fig-diffu-pde1-FE-fig-F=0.25 width="800px"}

![Forward Euler scheme for $F=0.51$.](fig/plug_FE_F051){#fig-diffu-pde1-FE-fig-F=0.51 width="800px"}


Instead of a discontinuous initial condition we now try the smooth
Gaussian function for $I(x)$. A simulation for $F=0.5$
is shown in Figure @fig-diffu-pde1-FE-fig-gauss-F=0.5. Now the numerical solution
is smooth for all times, and this is true for any $F\leq 0.5$.

![Forward Euler scheme for $F=0.5$.](fig/gaussian_FE_F05){#fig-diffu-pde1-FE-fig-gauss-F=0.5 width="800px"}
...
sketched above.
As mentioned in Section @eq-diffu-pde1-FE,
...
## Experiments
{#sec-diffu-pde1-theta-experiments}

We can repeat the experiments from Section @eq-diffu-pde1-FE-experiments
...
![Backward Euler scheme for $F=0.5$.](fig/plug_BE_F05){#fig-diffu-pde1-BE-fig-F=0.5 width="800px"}

The Backward Euler scheme always produces smooth solutions for any $F$.
Figure @fig-diffu-pde1-BE-fig-F=0.5 shows one example.
...
Section @eq-diffu-pde1-analysis explains why such noise occur.
