...
drawing_tool.set_coordinate_system(
    t_min, t_max, u_min, 1.2 * u_max, axis=True, xkcd=False
)
...
for i, t in enumerate(t_mesh):
    ...
    if 0 <= i < len(t_mesh) - 1:
        ...
        v_points[i] = Composition(
            dict(
                circle=Circle(point(t + dt / 2, v_value), r).set_linecolor("black"),
                # v_point=Text(r'$v^%d+\frac{1}{2}}$' % i,
                v_point=Text(
                    r"$v_{%d/2}^{n+\frac{1}{2}}$" % (2 * i + 1),
                    point(t + dt / 2, v_value)
                    + (point(0, 3 * r) if i > 0 else point(-3 * r, 0)),
                ),
            )
        )
...
axes = Composition(
    dict(
        ...
        y=Axis(point(0, -b), 1.1 * u_max, "", rotation_angle=90),
    )
)
...
illustration = Composition(
    dict(u=u_discrete, v=v_discrete, mesh_u=u_nodes, mesh_v=v_nodes, axes=axes)
).set_name("staggered")
drawing_tool.erase()
