...
force_L = Arrow1(p, p + 0.7 * unit_vec(pb - p)).set_linecolor("black").set_linewidth(1)
force_R = Arrow1(p, p + 0.7 * unit_vec(pf - p)).set_linecolor("black").set_linewidth(1)
...
fnts = 18
u_values = Composition(
    [
        Text("$u_{i-1}$", pb + v, fontsize=fnts),
...
line_L1 = Line(p, pb).set_linestyle("dashed").set_linecolor("black").set_linewidth(1)
line_R1 = Line(p, pf).set_linestyle("dashed").set_linecolor("black").set_linewidth(1)
line_L2 = Line(pb, pb2).set_linestyle("dashed").set_linecolor("black").set_linewidth(1)
line_R2 = Line(pf, pf2).set_linestyle("dashed").set_linecolor("black").set_linewidth(1)
...
fig = Composition(dict(masses=masses, mesh=mesh, forces=forces, u_values=u_values))
