...
\begin{align}
u^{n+1}**{i,j} &= -u^{n-1}**{i,j} + 2u^n_{i,j} + \nonumber\\
&\quad C_x^2(
u^{n}**{i+1,j} - 2u^{n}**{i,j} + u^{n}_{i-1,j}) + C_y^2
(u^{n}**{i,j+1} - 2u^{n}**{i,j} + u^{n}**{i,j-1}) + \Delta t^2 f**{i,j}^n,
...
For the first step, $n=0$, (@sec-wave-2D3D-impl1-2Du0-ueq-discrete)
...
\begin{align}
u^{1}**{i,j} &= u^0**{i,j} + \Delta t V_{i,j} + \nonumber\\
&\quad {\half}C_x^2(
u^{0}**{i+1,j} - 2u^{0}**{i,j} + u^{0}_{i-1,j}) + {\half}C_y^2
(u^{0}**{i,j+1} - 2u^{0}**{i,j} + u^{0}_{i,j-1}) +\nonumber\\
...
The algorithm is very similar to the one in 1D:
 ...
 1. Compute $u^1_{i,j}$ from (@sec-wave-2D3D-impl1-2Du0-ueq-discrete)
 1. Set $u^1_{i,j}=0$ for the boundaries $i=0,N_x$, $j=0,N_y$
 1. For $n=1,2,\ldots,N_t$:
  1. Find $u^{n+1}_{i,j}$ from (@sec-wave-2D3D-impl1-2Du0-ueq-discrete)
...
The key finite difference formula (@sec-wave-2D3D-models-unp1)
...
(@sec-wave-pde2-fd-verify-quadratic-uex):
...
![Snapshots of the surface plotted by Gnuplot.](fig/wave2D_u0_gnuplot_gaussian.png){#fig-wave2D3D-impl-viz-fig-gnuplot1 width="800px"}
...
(@sec-wave2D3D-impl-verify-quadratic) is indeed a solution of the
...
{#sec-wave-app-exer-wave2D-Neumann}
file=wave2D_dn
...
in the `pulse` function from Section @eq-wave-pde2-software
...
{#sec-wave-exer-3D-f77-cy-efficiency}
file=wave3D_u0
