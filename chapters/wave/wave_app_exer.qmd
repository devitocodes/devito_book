---
title: "Wave App Exer.Do"
---

{#sec-wave-app-exer}

## Exercise: Simulate waves on a non-homogeneous string
{#sec-wave-app-exer-string-discont}
...
{#sec-wave-app-exer-rod}
file=wave_rod

A hammer hits the end of an elastic rod. The exercise is to simulate
the resulting wave motion using the model (@sec-wave-app-elastic-rod-eq)
...
{#sec-wave-app-exer-tsunami1D-hill}
file=tsunami1D_hill
...
![Sketch of initial water surface due to a subsea earthquake.](fig/earthquake_tsunami_flat){#fig-wave-app-fig-1D-tsunami-flat width="800px"}

![An initial surface elevation is split into two waves.](fig/earthquake_tsunami_2waves){#fig-wave-app-fig-1D-tsunami-2waves width="800px"}
...
![Sketch of an earthquake-generated tsunami passing over a subsea hill.](fig/earthquake_tsunami_hill){#fig-wave-app-fig-1D-tsunami-hill width="800px"}
...
The [`wave1D_dn_vc.py`](https://github.com/hplgit/fdm-book/tree/master/src/wave/wave1D/wave1D_dn_vc.py)
...
(@sec-wave-app-exer-tsunami1D-hill-Gauss),
(@sec-wave-app-exer-tsunami1D-hill-cohat),
(@sec-wave-app-exer-tsunami1D-hill-box), or $B(x)=B_0$ (flat).
...
{#sec-wave-app-exer-tsunami2D-hill}
file=tsunami2D_hill
...
B_{my})$, generalizes (@sec-wave-app-exer-tsunami1D-hill-Gauss):
...
The ``cosine hat'' (@sec-wave-app-exer-tsunami1D-hill-cohat) can also be
generalized to
...
{#sec-wave-app-exer-tsunami-hill-viz-matplotlib}
...
{#sec-wave-app-exer-tsunami-hill-viz-packages}
...
{#sec-wave-app-exer-tsunami2D-hill-compiled}
...
{#sec-wave-app-exer-seismic2D}
file=seismic2D

The goal of this exercise is to simulate seismic waves using the
PDE model (@sec-wave-app-acoustic-seismic-acoustic1a) in a
...
equation (@sec-wave-app-exer-advec1D-ueq) arises in *transport*
...
A widely used numerical scheme for (@sec-wave-app-exer-advec1D-ueq)
...
C'(x) = \frac{1}{c(x)},
$$
is a solution of (@sec-wave-app-exer-advec1D-ueq) for any
...
To compute (@sec-wave-app-exer-advec1D-u-avar-sol) we need to integrate
...
We observe that (@sec-wave-app-exer-advec1D-C-recursion) is a
*difference equation* which we can solve by repeatedly
applying (@sec-wave-app-exer-advec1D-C-recursion) for
...
(@sec-wave-app-exer-advec1D-I-sin) and (@sec-wave-app-exer-advec1D-I-gauss).
...
and the velocity of the medium as
 ...
 1. $c(x) = 1 + I(x)$, where $I$ is given by
   (@sec-wave-app-exer-advec1D-I-sin) or (@sec-wave-app-exer-advec1D-I-gauss).
...
{#sec-wave-app-exer-anal-damped-wave1D}
file=damped_wave1D
...
{#sec-wave-app-exer-anal-damped-wave2D}
file=damped_wave2D
