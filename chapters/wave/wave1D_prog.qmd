...
Sections @eq-wave-string-alg and @sec-wave-string-impl can be
...
{#sec-wave-pde1-impl-verify-quadratic}

We use the test problem derived in Section @eq-wave-pde2-fd for
...
the various results in Section @eq-wave-pde1-analysis.
...
@eq-wave-pde2-fd-standing-waves, we can call `convergence_rates` to
...
`tmp_%04d.png`.
Section @eq-vib-ode1-anim contains more basic
...
make video files.
Section @eq-vib-ode1-anim presents basic information on how to
...
As mentioned in Section @eq-vib-ode1-anim, only
...
{#sec-wave-pde1-impl-vec-slices-basics}
...
$(u_1,u_2,\ldots,u_n)$, see Figure @eq-wave-pde1-vec-fig1.
...
{#sec-wave-pde1-impl-vec-slices-fdm}

We now have the necessary tools to vectorize the wave equation
algorithm as described mathematically in Section @eq-wave-string-alg
...
## Verification
{#sec-wave-pde1-impl-vec-verify-quadratic}
...
## Exercise: Simulate a standing wave
{#sec-wave-exer-standingwave}
file=wave_standing
...
in space, see Section @eq-wave-pde1-num-dispersion for explanation).
!eremarks

## Exercise: Add storage of solution in a user action function
{#sec-wave-exer-store-list}
...
## Exercise: Use a class for the user action function
{#sec-wave-exer-store-list-class}
file=wave1D_u0_s2c
...
## Exercise: Compare several Courant numbers in one movie
{#sec-wave-exer-multiple-C}
...
## Exercise: Implementing the solver function as a generator
{#sec-wave-exer-useraction-generator}
...
## Project: Calculus with 1D mesh functions
{#sec-wave-exer-mesh1D-calculus}
