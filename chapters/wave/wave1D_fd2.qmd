...
the wave as a mirror and preserves the sign, see a [web page](http://hplgit.github.io/fdm-book/doc/pub/book/html/mov-wave/demo_BC_gaussian/index.html) or a
"movie file":
"{{< var docraw >}}/mov-wave/demo_BC_gaussian/movie.flv" for
demonstration.
...
an animation in a [web page](http://hplgit.github.io/fdm-book/doc/pub/book/html/mov-wave/N_stencil_gpl/index.html)
or a "movie file":
"{{< var docraw >}}/mov-wave/N_stencil_gpl/movie.ogg".


## Implementation of Neumann conditions
{#sec-wave-pde2-Neumann-impl}
...
the exact solution of the PDE, see Section @eq-wave-pde1-num-dispersion).
...
Sections @eq-wave-pde2-fd and @sec-wave-pde1-impl-verify-quadratic,
...
A better solution is to use the ideas of Section @eq-wave-indexset:
...
![Left: wave entering another medium; right: transmitted and reflected wave.](fig/pulse1_in_two_media){#fig-wave-pde1-fig-pulse1-two-media width="800px"}
...
and equation (@sec-wave-app-string-model2).
...
nonlinearities (see Section @eq-vib-ode2-fdm-fquad for an example).
...
{#sec-wave-pde2-var-c-stability}

The stability criterion derived later (Section @eq-wave-pde1-stability)
...
u_{i+1}^n = u_{i-1}^n,
$$
for $i=N_x$. Using the scheme (@sec-wave-pde2-var-c-scheme-impl)
...
(@sec-wave-pde2-var-c-scheme-impl-Neumann0), leading to the term
...
implementation of the scheme (@sec-wave-pde2-var-c-scheme-impl):
...
This technique is further explained in Section @eq-softeng2-wave1D-filestorage-hash.
...
ref[Section @eq-softeng2-wave1D-filestorage][ in
...
{#sec-wave-exer-standingwave-damped-uex}
file=damped_waves
...
{#sec-wave-app-exer-pulse1D-analysis}
...
{#sec-wave-app-exer-pulse1D-harmonic}
...
Show that (@sec-wave-app-exer-radiationBC-eq) accepts
a solution $u = g_R(x-ct)$ (right-going wave),
but not $u = g_L(x+ct)$ (left-going wave). This means
that (@sec-wave-app-exer-radiationBC-eq) will allow any
...
the condition (@sec-wave-app-exer-radiationBC-eq)
...
where you have $u_x=0$ at $x=0$, the condition (@sec-wave-app-exer-radiationBC-eq)
...
(@sec-wave-app-exer-radiationBC-eq-op-1storder).
...
discretized as shown in (@sec-wave-pde2-var-c-scheme-impl-Neumann)
and (@sec-wave-pde2-var-c-scheme-impl-Neumann2).
...
(@sec-wave-pde2-var-c-scheme-impl-Neumann).
!esubex
...
of the scheme
(@sec-wave-pde2-var-c-scheme-impl-Neumann2). Now,
...
{#sec-wave-fd2-exer-verify-cubic}
