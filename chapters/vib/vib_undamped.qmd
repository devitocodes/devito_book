...
Section @sec-vib-app-mass-spring derives (@eq-vib-ode1) from physical
...
listed in Appendix @sec-sec-form-fdop
...
problem, because theory (from Section @eq-vib-ode1-analysis) shows
...
The error analysis in Section @eq-vib-ode1-analysis is quite
detailed and suggests that $r=2$.
##but other methods like truncation error
##analysis
##% if BOOK == "book":
##(see Appendix @eq-trunc-vib-undamped)
##% endif
##also points to $r=2$,
...
{#sec-vib-ode1-longseries}

Figure @eq-vib-ode1-2dt shows a comparison of the exact and numerical
...
![Effect of halving the time step.](fig/vib_freq_err1){#fig-vib-ode1-2dt width="800px"}
...
displaced $t$ axes!):](fig/bokeh_gridplot1){width="800px"}

![A function for creating a Bokeh plot, given a list of `u` arrays
and corresponding `t` arrays, is implemented below.](fig/bokeh_gridplot2){width="800px"}
...
{#sec-vib-ode1-empirical}

For oscillating functions like those in Figure @eq-vib-ode1-2dt we may
...
![Empirical angular frequency (left) and amplitude (right) for three different time steps.](fig/empirical_ampl_freq){#fig-vib-ode1-fig-freq-ampl width="800px"}


{#sec-vib-ode1-analysis}
...
left plot in Figure @eq-vib-ode1-2dt.

Figure @eq-vib-ode1-tildeomega-plot plots the discrete frequency
...
analysis as explained in Appendix @eq-trunc-vib-undamped.
...
growing solutions. Figure @tbl-vib-ode1-dt-unstable
...
severe: Figure @eq-vib-ode1-dt-stablimit shows a comparison of the
...
observations in Figure @eq-vib-ode1-dt-stablimit that the numerical
...
:::{.callout-important title="From the accuracy and stability"}
...
(@sec-vib-model2x2-ueq)-(@sec-vib-model2x2-veq) becomes
...
Figure @eq-vib-ode1-1st-odespy-theta-phaseplane show the
...
@eq-vib-ode1-1st-odespy-theta-phaseplane has a pronounced spiral
curve, pointing to the fact that the amplitude steadily grows, which
is also evident in Figure @eq-vib-ode1-1st-odespy-theta.  The
...
![Comparison of classical schemes in the phase plane for two time step values.](fig/vib_theta_1_pp){#fig-vib-ode1-1st-odespy-theta-phaseplane width="800px"}

![Comparison of solution curves for classical schemes.](fig/vib_theta_1_u){#fig-vib-ode1-1st-odespy-theta width="800px"}
...
perform. Figures @eq-vib-ode1-1st-odespy-RK-phaseplane and
@eq-vib-ode1-1st-odespy-RK show the solutions with larger $\Delta t$
values than what was used in the previous two plots.

![Comparison of Runge-Kutta schemes in the phase plane.](fig/vib_RK_1_pp){#fig-vib-ode1-1st-odespy-RK-phaseplane width="800px"}

![Comparison of Runge-Kutta schemes.](fig/vib_RK_1_u){#fig-vib-ode1-1st-odespy-RK width="800px"}
...
Figure @eq-vib-ode1-1st-odespy-CN-long-phaseplane.  It is clear that
...
![Long-time behavior of the Crank-Nicolson scheme in the phase plane.](fig/vib_CN_10_pp){#fig-vib-ode1-1st-odespy-CN-long-phaseplane width="800px"}

## This one has different types of plot compared...
##![Long-time behavior of the Crank-Nicolson scheme.](fig/vib_CN_10_u){#fig-vib-ode1-1st-odespy-CN-long width="800px"}
...
in the system, as we do in Section @sec-vib-model2.)
...
(computed by the [`vib_undamped_odespy.py`](https://github.com/hplgit/fdm-book/tree/master/src/vib/vib_undamped_odespy.py) program). The test problem is
...
$$
(see formula (@eq-form-exp-fd1c-center) in Appendix @sec-sec-form-fdexp),
we get
...
(@sec-vib-model2x2-EulerCromer-ueq1), it may be used directly
in
(@sec-vib-model2x2-EulerCromer-veq1) to compute $v^{n+1}$.
...
(@sec-vib-model2x2-EulerCromer-veq1b)-(@sec-vib-model2x2-EulerCromer-ueq1b)
...
{#sec-vib-model2x2-EulerCromer-equiv}
...
(@sec-vib-model2x2-EulerCromer-ueq1)-(@sec-vib-model2x2-EulerCromer-veq1)
or
(@sec-vib-model2x2-EulerCromer-veq1b)-(@sec-vib-model2x2-EulerCromer-ueq1b).
The $v^{n+1}$ term in (@sec-vib-model2x2-EulerCromer-veq1b) can
be eliminated from (@sec-vib-model2x2-EulerCromer-ueq1b):
...
using (@sec-vib-model2x2-EulerCromer-ueq1b):
$$
v^{n} = \frac{u^n - u^{n-1}}{\Delta t},
$$
and when this is inserted in (@sec-vib-model2x2-EulerCromer-elim1) we get
$$
...
(@sec-vib-model2x2-EulerCromer-veq1b) we get

$$
v^1=v^0-\Delta t\omega^2 u^0
=\Delta t\omega^2 u^0,
$$
and from (@sec-vib-model2x2-EulerCromer-ueq1b)
it follows that
...
(@sec-vib-model2x2-EulerCromer-ueq1b)-(@sec-vib-model2x2-EulerCromer-veq1b)
corresponds to using only a first-order approximation to the initial
condition in the method from Section @eq-vib-ode1-fdm.
...
@eq-vib-ode1-fdm.  The error is $\frac{1}{2}\omega^2\Delta t^2 u^0$.

## Implementation
{#sec-vib-model2x2-EulerCromer-impl}
...
@eq-vib-ode1-analysis-sol is not a solution of the Euler-Cromer
scheme!
...
@eq-vib-ode1-analysis-sol. This adjustment is done in an alternative
...
@eq-vib-ode1-analysis-numfreq, gives a 4th-order method there, while
...
{#sec-vib-model2x2-StormerVerlet}
...
rounding errors, is
(@sec-vib-model2x2-StormerVerlet-eqv)-(@sec-vib-model2x2-StormerVerlet-equ).
...
Figure @eq-staggered-EC-fig1 presents a graphical sketch of two
...
$$
Using the discretized equation (@sec-vib-model2x2-EulerCromer-veq1s2) for
$n=0$ yields
...
The algorithm goes like this:
 ...
 1. For $n=1,2,\ldots$:
   1. Compute $u^{n}$ from (@sec-vib-model2x2-EulerCromer-ueq1s2).
   1. Compute $v^{n+\half}$ from (@sec-vib-model2x2-EulerCromer-veq1s2).

### Implementation with integer indices
Translating the schemes (@sec-vib-model2x2-EulerCromer-ueq1s2)
and (@sec-vib-model2x2-EulerCromer-veq1s2) to computer code
...
{#sec-vib-exer-undamped-verify-linquad}
...
{#sec-vib-exer-undamped-adaptive}
...
method from Section @eq-vib-ode1-fdm with the same number of (equally
...
![!esol](fig/vib_undamped_adaptive){width="800px"}


## Exercise: Use a Taylor polynomial to compute $u^1$
{#sec-vib-exer-step4b-alt}
file=vib_first_step
...
as the `solver` function from Section @sec-vib-impl1-solver (if `return_v`
...
![!esol](fig/error_fraction){width="600px"}


## Exercise: Verify convergence rates of the error in energy
{#sec-vib-exer-energy-convrate}
...
{#sec-vib-exer-verify-gen-linear}
file=vib_verify_mms
...
@eq-vib-ode2-fdm-flin. Discretizing the equation according to
...
text in Chapter @eq-vib-ode2-fdm-fquad. Discretizing the
...
Proceeding with linear damping, we get from chapter @sec-vib-ode2-verify that
$$
F(t) = bV + c(Vt + I)\tp
\nonumber
$$
...
If the damping is rather quadratic, we find from chapter @sec-vib-ode2-verify that
$$
F(t) = b|V|V + c(Vt + I)\tp
\nonumber
$$
...
## Problem: Minimize memory usage of a simple vibration solver
{#sec-vib-exer-memsave0}
file=vib_memsave0
...
## Problem: Minimize memory usage of a general vibration solver
{#sec-vib-exer-memsave}
file=vib_memsave
...
!bsubex
Implement the Euler-Cromer method from Section @eq-vib-ode2-EulerCromer.
...
from Section @eq-vib-ode2-fdm-flin and the Euler-Cromer scheme
...
from Section @eq-vib-ode2-fdm-flin. A suitable function for making
...
{#sec-vib-exer-EulerCromer-analysis}
...
for $u^n$ coincides with that found in Section @eq-vib-ode1-analysis.
...
!bsol
We follow the ideas in Section @eq-vib-ode1-analysis. Inserting
...
\end{align} \tp
Solving (@sec-vib-exer-EulerCromer-analysis-eqv) with respect to $q$ gives

$$
q = \frac{1}{\Delta t}\left( 1 - \exp{(i\tilde\omega\Delta t)} \right)\tp
$$
Inserting this expression for $q$ in (@sec-vib-exer-EulerCromer-analysis-equ)
results in
...
Section @eq-vib-ode1-analysis, such that $\tilde\omega$ is the
same. The accuracy, stability, and formula for the exact discrete solution
are then all the same as derived in Section @eq-vib-ode1-analysis.
!esol
