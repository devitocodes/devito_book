...
{#sec-nonlin-exer-relax-formula}
file=relaxed_Newton

Derive (@eq-nonlin-timediscrete-logistic-relaxation-Newton-formula)
in Section @eq-nonlin-timediscrete-logistic-relaxation.

## Problem: Derive and investigate a generalized logistic model
{#sec-nonlin-exer-logistic-gen}
file=logistic_p
...
scheme for (@sec-nonlin-exer-logistic-gen-eq).
...
$p=1$ and the choice (@sec-nonlin-exer-logistic-gen-r1).

!bsol
We specialize the code for $a(u)$ to (@sec-nonlin-exer-logistic-gen-r1)
...
the model (@sec-nonlin-exer-logistic-gen-r1).
See how sensitive
...
{#sec-nonlin-exer-Newton-problems1}
...
## Exercise: Compute the Jacobian of a $2\times 2$ system
{#sec-nonlin-exer-vib-Jacobian}
...
{#sec-nonlin-exer-vib-geometric-mean}
file=nonlin_vib
...
{#sec-nonlin-exer-products-arith-mean}
file=product_arith_mean

In Section @eq-nonlin-pdelevel-Picard-CN we introduce alternative
...
$$
A fundamental question is whether (@sec-nonlin-exer-products-arith-mean-f)
and (@sec-nonlin-exer-products-arith-mean-p) have different
...
(@sec-nonlin-exer-products-arith-mean-f) and
(@sec-nonlin-exer-products-arith-mean-p) to see what the
...
(@sec-nonlin-exer-products-arith-mean-f) and
(@sec-nonlin-exer-products-arith-mean-p) for $n=0$. (Choosing $n=0$
...
## Problem: Discretize a 1D problem with a nonlinear coefficient
{#sec-nonlin-exer-1D-1pu2-fem}
...
!bsubex
Construct a Picard iteration method for (@sec-nonlin-exer-1D-1pu2-PicardNewton-pde)
without discretizing in space.
!esubex

!bsubex
Apply Newton's method to (@sec-nonlin-exer-1D-1pu2-PicardNewton-pde)
without discretizing in space.
!esubex

!bsubex
Discretize (@sec-nonlin-exer-1D-1pu2-PicardNewton-pde) by a centered finite
...
{#sec-nonlin-exer-1D-fu-discretize-fd}
...
The problem (@sec-nonlin-exer-1D-fu-discretize-fd-pde) has an
exact solution
...
$$
There are two solutions of (@sec-nonlin-exer-1D-fu-discretize-fd-pde) for
...
!bsubex
Discretize (@sec-nonlin-exer-1D-fu-discretize-fd-pde) by a
...
{#sec-nonlin-exer-1D-heat-nonlinear-fdm}
...
{#sec-nonlin-exer-2D-heat-nonlinear-fd}
file=nonlin_heat_FD_CN_2D

Redo Section @eq-nonlin-alglevel-dD-fd when a Crank-Nicolson scheme
...
## Problem: Find the sparsity of the Jacobian
{#sec-nonlin-exer-sparsity-Jacobian}
...
{#sec-nonlin-exer-continuation-1DnNflow}
