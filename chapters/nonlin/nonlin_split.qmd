## Operator splitting methods {#sec-nonlin-splitting}
...
## Ordinary operator splitting for ODEs {#sec-nonlin-splitting-ODE}
...
$$ {#eq-nonlin-splitting-ODE-step1}
$$
u(t_{n+1}) = u^\stepzero e^{b\Delta t}
$$ {#eq-nonlin-splitting-ODE-step2}
The first step (@eq-nonlin-splitting-ODE-step1) means solving
$u'=f_0$ over a time interval $\Delta t$ with $u(t_n)$ as start value.
The second step (@eq-nonlin-splitting-ODE-step2) means solving
...
## Strange splitting for ODEs {#sec-nonlin-splitting-ODE-Strange}

The accuracy of the splitting method in Section @sec-nonlin-splitting-ODE
can be improved from $\Oof{\Delta t}$ to $\Oof{\Delta t^2}$ using so-called
*Strange splitting*, where we take half a step with the $f_0$ operator,
...
t}$, but for Strange splitting it makes sense to use schemes of order
$\Oof{\Delta t^2}$.

With the notation introduced for Strange splitting, we may express
...
## Example: Logistic growth {#sec-nonlin-splitting-logistic}
...
[`split_logistic.py`](https://github.com/hplgit/fdm-book/tree/master/src/nonlin/split_logistic.py).
...
Figure @fig-nonlin-splitting-ODE-logistic-fig shows that the impact of
...
accurate than any of the splitting schemes. We also see that Strange
...
@fig-nonlin-splitting-ODE-logistic-fig), the asymptotic values are off
by 20-30%.  A more reasonable time step ($\Delta t = 0.05$, right plot
in Figure @fig-nonlin-splitting-ODE-logistic-fig) gives better
...
$$
$$
\frac{\partial u^{\stepone}}{\partial t} = f(u^{\stepone})\tp
$$ {#eq-nonlin-splitting-RD-eq-diffu}
Looking at these familiar problems,
we may apply a $\theta$ rule (implicit) scheme for
(@eq-nonlin-splitting-RD-eq-diffu) over one time step
and avoid dealing with nonlinearities
by applying an explicit scheme for (@eq-nonlin-splitting-RD-eq-diffu)
...
(@eq-nonlin-splitting-RD-eq-diffu) by
$$
[D_t u^\stepzero =
...
is maximum $\Oof{\Delta t^2}$ for Strange splitting,
...
The algorithm goes like this:
 ...
 1. Solve the reaction ODEs at each mesh point in $[t_n,t_n+\Delta t]$,
...
## Example: Reaction-Diffusion with linear reaction term {#sec-nonlin-splitting-RD-linearR}
...
straight Forward Euler), ordinary splitting, first order Strange
splitting, and second order Strange splitting.  In all four methods, a
...
[`split_diffu_react.py`](https://github.com/hplgit/fdm-book/tree/master/src/nonlin/split_diffu_react.py). When executed,
...
scheme, the ordinary splitting scheme and first order Strange splitting
...
For the second order Strange splitting scheme, we have $r = 2$ and a
...
during ordinary splitting, while Strange splitting (both first and
...
For the two Strange splitting approaches, each time step `dt` is
...
once again for (the remaining) `dt/2`. Since first order Strange
...
The second order version of the Strange splitting approach utilizes a
...
structure as the one for first order Strange splitting and was
implemented as
...
convergence rates are as expected. The second order Strange splitting
...
We refer to ref[Section @sec-diffu-pde1-analysis][ in
[@Langtangen_Linge]][the section "Analysis of schemes for
the diffusion equation": "" in [@Langtangen_Linge]] and to
the book [@Langtangen_decay] for a discussion of exact numerical
