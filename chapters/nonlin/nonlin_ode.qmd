## Linear versus nonlinear equations {#sec-nonlin-timediscrete-logistic}
...
:::{.callout-note title="Mathematical proof of linearity"}
...
time discretization of (@eq-nonlin-timediscrete-logistic-eq),
...
using (@eq-nonlin-timediscrete-logistic-eq) as model. We shall go through
...
## Linearization by explicit time discretization {#sec-nonlin-timediscrete-logistic-FE}
...
(@eq-nonlin-timediscrete-logistic-eq), sampled at $t=t_n$, it results in
$$
...
for (@eq-nonlin-timediscrete-logistic-eq) leads to the following
formulas:
...
## Exact solution of nonlinear algebraic equations {#sec-nonlin-timediscrete-logistic-roots}

Switching to a Backward Euler scheme for
(@eq-nonlin-timediscrete-logistic-eq),
$$
\frac{u^{n} - u^{n-1}}{\Delta t} = u^n(1 - u^n),
$$ {#eq-nonlin-timediscrete-logistic-eq-BE}
...
$$ {#eq-nonlin-timediscrete-logistic-eq-F}
The solution is readily found to be
$$
u = \frac{1}{2\Delta t}
\left(-1+\Delta t \pm \sqrt{(1-\Delta t)^2 - 4\Delta t u^{(1)}}\right) \tp
...
## Picard iteration {#sec-nonlin-timediscrete-logistic-Picard}
...
$$ {#eq-nonlin-timediscrete-logistic-BE-Picard-1it}
...
## Linearization by a geometric mean {#sec-nonlin-timediscrete-logistic-geometric-mean}
...
## Newton's method {#sec-nonlin-timediscrete-logistic-Newton}
...
Exercise @sec-nonlin-exer-Newton-problems1 to get a better understanding
...
u^{-}\ \leftarrow u\tp
$$ {#eq-nonlin-timediscrete-logistic-Newton-alg1}
...
$$ {#eq-nonlin-timediscrete-logistic-Newton-alg2}
for $k=0,1,\ldots$.
...
## Relaxation {#sec-nonlin-timediscrete-logistic-relaxation}
...
$$ {#eq-nonlin-timediscrete-logistic-relaxation-Newton-formula}

## Implementation and experiments {#sec-nonlin-timediscrete-logistic-impl}

The program [`logistic.py`](https://github.com/devitocodes/devito_book/tree/main/src/nonlin/logistic.py) contains
...
Figure @fig-nonlin-timediscrete-logistic-impl-fig-u. The one to
...
@fig-nonlin-timediscrete-logistic-impl-fig-iter displays the number of
...
:::{.callout-note title="Explicit vs implicit treatment of nonlinear terms"}
...
(Experiments can be done with the code [`ODE_Picard_tricks.py`](https://github.com/devitocodes/devito_book/tree/main/src/nonlin/ODE_Picard_tricks.py).)
:::
...
\frac{\partial}{\partial u}f(u^{-},t_n)} \tp
$$ {#eq-nonlin-ode-generic-Newton}
...
(@eq-nonlin-ode-generic-Newton) with $F(u)$ given in
(@eq-nonlin-ode-generic-Newton2) and
$$
F^{\prime}(u)= 1 - \half\Delta t\frac{\partial f}{\partial u}(u,t_{n+1})\tp
$$

## Systems of ODEs {#sec-nonlin-ode-generic-sys-pendulum}
...
The system at the end of Section @sec-nonlin-ode-generic-sys-pendulum fits
this notation with $N=1$, $F_0(u)$ given by the left-hand side of
(@eq-nonlin-ode-generic-sys-pendulum-u0), while $F_1(u)$ is
the left-hand side of (@eq-nonlin-ode-generic-sys-pendulum-u1).
...
## Picard iteration {#sec-nonlin-systems-alg-Picard}
...
## Newton's method {#sec-nonlin-systems-alg-Newton}
...
:::{.callout-note title="Algorithm for Newton's method"}
...
:::{.callout-note title="Combined algorithm for Picard and Newton iteration"}
...
## Stopping criteria {#sec-nonlin-systems-alg-terminate}
...
$$ {#eq-nonlin-systems-alg-SI-CN-FS}
$$
F_I(S,I) = I - I^{(1)} -
\half\Delta t\beta(S^{(1)}I^{(1)} + SI) +
\half\Delta t\nu(I^{(1)} + I) =0\tp
$$ {#eq-nonlin-systems-alg-SI-CN-FI}
