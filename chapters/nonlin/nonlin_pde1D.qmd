...
algebraic equations.  In Section @eq-nonlin-alglevel-1D we shall take
...
Section @eq-nonlin-systems-alg.  Very often, the two approaches are
...
{#sec-nonlin-pdelevel-explicit}

The nonlinearities in the PDE are trivial to deal with if we choose an
explicit time integration method for (@sec-nonlin-pdelevel-model-pde),
...
{#sec-nonlin-pdelevel-Picard}

A Backward Euler scheme for (@sec-nonlin-pdelevel-model-pde)
reads
...
{#sec-nonlin-pdelevel-Picard-CN}

A Crank-Nicolson discretization of
(@sec-nonlin-pdelevel-model-pde) applies a centered difference
at $t_{n+\frac{1}{2}}$:
...
{#sec-nonlin-alglevel-1D}

Section @eq-nonlin-pdelevel presented methods for linearizing
...
Newton's method as presented in Section @eq-nonlin-systems-alg.
...
Section @eq-nonlin-systems-alg. With the notation $u^{-}$ for the
