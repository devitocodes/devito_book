## Nonlinear diffusion model {#sec-nonlin-pdelevel}
...
algebraic equations.  In Section @sec-nonlin-alglevel-1D we shall take
...
Section @sec-nonlin-systems-alg.  Very often, the two approaches are
...
## Explicit time integration {#sec-nonlin-pdelevel-explicit}

The nonlinearities in the PDE are trivial to deal with if we choose an
explicit time integration method for the nonlinear diffusion equation,
...
## Backward Euler scheme and Picard iteration {#sec-nonlin-pdelevel-Picard}

A Backward Euler scheme for the nonlinear diffusion equation
reads
$$
[D_t^- u = \nabla\cdot (\dfc(u)\nabla u) + f(u)]^n\tp
$$
...
\nabla u^{n,k+1})
+ f(u^{n,k})\tp
$$ {#eq-nonlin-pdelevel-pde-BE-Picard-k}
...
\nabla u)
+ f(u^{-})\tp
$$ {#eq-nonlin-pdelevel-pde-BE-Picard}
...
:::{.callout-note title="Remark on notation"}
...
## Backward Euler scheme and Newton's method {#sec-nonlin-pdelevel-Newton}

At time level $n$, we have to solve the stationary PDE
(@eq-nonlin-pdelevel-pde-BE). In the previous section, we
...
$$
u^{n} = u^{n,k} + \delta u\tp
$$ {#eq-nonlin-pdelevel-Newton-ansatz}
...
linear in $\delta u$ (which makes (@eq-nonlin-pdelevel-Newton-ansatz)
an approximation for $u^{n}$). Then we can solve a linear PDE for
the correction $\delta u$ and use (@eq-nonlin-pdelevel-Newton-ansatz)
...
+ f(u^{n,k}+\delta u)\tp
$$ {#eq-nonlin-pdelevel-pde-BE-Newton1}
...
\end{split}
$$ {#eq-nonlin-pdelevel-pde-BE-Newton2}
...
+ f^{\prime}(u^{n,k})\delta u\tp
\end{split}
$$ {#eq-nonlin-pdelevel-pde-BE-Newton2-F}
...
+ f^{\prime}(u^{-})\delta u\tp
\end{split}
$$ {#eq-nonlin-pdelevel-pde-BE-Newton2-F2}
...
## Crank-Nicolson discretization {#sec-nonlin-pdelevel-Picard-CN}

A Crank-Nicolson discretization of
the nonlinear diffusion equation applies a centered difference
at $t_{n+\frac{1}{2}}$:
$$
...
mean of products. Exercise @sec-nonlin-exer-products-arith-mean investigates
this question, and the answer is that the approximation is
$\Oof{\Delta t^2}$ in both cases.

## Discretization in space and Newton's method {#sec-nonlin-alglevel-1D}

Section @sec-nonlin-pdelevel presented methods for linearizing
...
Newton's method as presented in Section @sec-nonlin-systems-alg.
...
\quad \dfc(u(0))u^{\prime}(0) = C,\ u(L)=D \tp
$$ {#eq-nonlin-alglevel-1D-pde}
...
$$ {#eq-nonlin-alglevel-1D-pde-tver}
...
scheme for (@eq-nonlin-alglevel-1D-pde-tver) leads to
$$
\frac{u^{n}-u^{n-1}}{\Delta t} =
\frac{d}{dx}\left(
\alpha(u^n)\frac{du^n}{dx}\right) - au^n + f(u^n)\tp
$$ {#eq-nonlin-alglevel-1D-pde-tver-BE}
Introducing $u(x)$ for $u^n(x)$, $u^{(1)}$ for $u^{n-1}$, and defining $f(u)$
in (@eq-nonlin-alglevel-1D-pde) to be $f(u)$ in
(@eq-nonlin-alglevel-1D-pde-tver-BE) plus $u^{n-1}/\Delta t$, gives
(@eq-nonlin-alglevel-1D-pde) with $a=1/\Delta t$.

## Finite difference discretization {#sec-nonlin-alglevel-1D-fd}

The nonlinearity in the differential equation
(@eq-nonlin-alglevel-1D-pde) poses no more difficulty than a variable
...
$$ {#eq-nonlin-alglevel-1D-fd-dfc-mean-u}
...
of (@eq-nonlin-alglevel-1D-fd-deq) for $i=0$.
...
we use a known $u_{-1}$ value in (@eq-nonlin-alglevel-1D-fd-deq)
...
$$ {#eq-nonlin-alglevel-1D-fd-Neumann-x0-um1}
...
Section @sec-nonlin-systems-alg. With the notation $u^{-}$ for the
...
$$ {#eq-nonlin-alglevel-1D-fd-2x2-x0}
$$
A_{1,0}u_{0} + A_{1,1}u_1 + A_{1,2}u_2 = b_1\tp
$$ {#eq-nonlin-alglevel-1D-fd-2x2-x1}
Equation (@eq-nonlin-alglevel-1D-fd-2x2-x0) written out reads
...
$$
Equation (@eq-nonlin-alglevel-1D-fd-2x2-x1) contains the unknown $u_2$
for which we have a Dirichlet condition. In case we omit the
condition as a separate equation, (@eq-nonlin-alglevel-1D-fd-2x2-x1)
